...
var blockTextures map[world.BlockType]*BlockTexture
...
func GetTexLayer(blockType world.BlockType, face world.BlockFace) int32 {
    ...
    return blockTex.GetTextureFace(face).layer
}
...
func createTextureArray() {
    ...
    gl.TexStorage3D(gl.TEXTURE_2D_ARRAY, 1, gl.RGBA8, texSize, texSize, int32(len(blockTextures)))

    var layer int32 = 0
    for blockType, tex := range blockTextures {
        for face, subTex := range tex.faces {
            def := "BlockType " + blockType.String() + ", face " + face.String()
            img, _, err := image.Decode(bytes.NewReader(subTex.bin))
            if err != nil {
                panic("Bad texture for " + def + "\n" + err.Error())
            }
            rgba := image.NewRGBA(img.Bounds())
            if rgba.Stride != rgba.Rect.Size().X*4 {
                panic("Unsupported stride for " + def)
            }
            layer++
